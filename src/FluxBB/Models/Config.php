...
class Config
{
    ...
    protected static function loadData()
    {
        ...
        static::$data = static::$original = Cache::remember('fluxbb.config', 24 * 60, function () {
            $data = DB::table('fluxbb_config')->get();
            $cache = array();
    ...
    public static function save()
    {
        ...
        if (!empty($insert_values)) {
            DB::table('fluxbb_config')->insert($insert_values);
        }

        foreach ($changed as $name => $value) {
            DB::table('fluxbb_config')->where('conf_name', '=', $name)->update(array('conf_value' => $value));
        }

        // Deleted keys
        $deleted_keys = array_keys(array_diff_key(static::$original, static::$data));
        if (!empty($deleted_keys)) {
            DB::table('fluxbb_config')->whereIn('conf_name', $deleted_keys)->delete();
        }
